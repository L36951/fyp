...
const ManageSensors = () => {
  ...
  const [tableHeader, setTaableHeader] = useState(['#', 'Sensor Type', 'Symbol']);
  const [accessor, setAccessor] = useState(['sensortype', 'sensorname']);
  const [allTableBody, setAllTableBody] = useState([]);
  const [tableBodyHolder, setTableBodyHolder] = useState([]);
  const [tableBody, setTableBody] = useState([]);
  const [link, setLink] = useState([`${location.pathname}/edit`, "sensortype"]);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.REACT_APP_SECRET_APIPATH}api/sensortype/query`, {
          headers: {
            token: `${token}`,
          },
        })
